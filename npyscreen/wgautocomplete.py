...
class Autocomplete(textbox.Textfield):
    """Override auto_complete to add functionality.  See filename class for example"""
    ...
    def get_choice(self, values):
        # allows user to select from a list of values for auto_complete
...
class Filename(Autocomplete):
    def auto_complete(self, input):
        ...
        for i in range(1):
            ...
            possibilities = list(
                filter((lambda x: os.path.split(x)[1].startswith(fname)), flist)
            )
            ...
            if fname.startswith("."):
                filelist = list(
                    filter((lambda x: os.path.basename(x).startswith(".")), filelist)
                )
            else:
                filelist = list(
                    filter(
                        (lambda x: not os.path.basename(x).startswith(".")), filelist
                    )
                )

            for index1 in range(len(filelist)):
                if os.path.isdir(filelist[index1]) and not filelist[index1].endswith(
                    os.sep
                ):
